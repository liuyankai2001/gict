...
class Data():
    def __init__(self,target_compound):
        ...
        # if len(sys.argv) == 1:
        #     print(
        #         'Please, set project name as the first argument of the script "ex. python3 main.py scoulerine_case1a"')
        ...
        #     exit()
        self.projectname=target_compound
        ...
        self.network_file = project_config.DATA_DIR / 'network.csv'
        self.compounds_file = project_config.DATA_DIR / 'compounds.csv'
        self.reactions_file = project_config.DATA_DIR / 'reactions_pairs.csv'
        self.bridgit_predictions_file = project_config.DATA_DIR / 'bridgit_predictions.csv'
        self.source_compounds_file = project_config.PROJECT_DIR / self.projectname / 'source_compounds.txt'
        self.path_compounds_file = project_config.PROJECT_DIR / self.projectname / 'path_compounds.txt'
        self.excludelist_compounds = project_config.PROJECT_DIR / self.projectname / 'excludelists' / 'compounds.txt'
        self.excludelist_reactions = project_config.PROJECT_DIR / self.projectname / 'excludelists' / 'reactions.txt'
        self.pathway_file = project_config.OUTPUT_DIR / self.projectname / 'pathways_raw.txt'
        self.pathway_reactions_file = project_config.OUTPUT_DIR / self.projectname / 'pathway_reactions.txt'
        self.pathway_enzymes_file = project_config.OUTPUT_DIR / self.projectname / 'pathway_enzymes.txt'
        self.ranked_pathway_file = project_config.OUTPUT_DIR / self.projectname / 'pathways.csv'
        self.compound_limits_file = project_config.OUTPUT_DIR / self.projectname / 'compound_parameters.csv'
        self.gephifilesfolder = project_config.OUTPUT_DIR / self.projectname / 'visualization'
        # self.pw_img_folder = '../output/'+self.projectname+'/visualization/pathways_img'
        self.pw_img_folder = project_config.OUTPUT_DIR / self.projectname / 'visualization'

        # print(project_config.PROJECT_DIR / self.projectname / 'parameters.txt')
        # Setting the defaults in case the individual parameter files were not provided
        if os.path.exists(project_config.PROJECT_DIR / self.projectname / 'parameters.txt'):

            self.param_file =project_config.PROJECT_DIR / self.projectname / 'parameters.txt'
        else:
            self.param_file =project_config.DEFAULT_PARAMETERS_DIR / 'parameters.txt'

        if not os.path.exists(self.compound_limits_file):
            self.compound_limits_file = project_config.DEFAULT_PARAMETERS_DIR / 'compound_parameters.csv'

        if not os.path.exists(self.source_compounds_file):
            self.source_compounds_file = project_config.DEFAULT_PARAMETERS_DIR / 'source_compounds.csv'

        if not os.path.exists(self.excludelist_compounds):
            self.excludelist_compounds = project_config.DEFAULT_EXCLUDELISTS_DIR / "compounds.txt"

        if not os.path.exists(self.excludelist_reactions):
            self.excludelist_reactions = project_config.DEFAULT_EXCLUDELISTS_DIR / "reactions.txt"
        ...
        self.colorpalette = {0: "0, 76, 163", 1: "138, 81, 165", 2: "203, 94, 153", 3: "244, 123, 137",
                             4: "255, 164, 126",
                             5: "255, 210, 134", 6: "255, 255, 166", 7: "205, 204, 0", 8: "255, 255, 0",
                             9: "255, 204, 0", 10: "255, 102, 0", 11: "204, 0, 1"}

        # creating output directories
        if not os.path.exists(project_config.OUTPUT_DIR):
            os.mkdir(project_config.OUTPUT_DIR)
        if not os.path.exists(project_config.OUTPUT_DIR / self.projectname):
            os.mkdir(project_config.OUTPUT_DIR / self.projectname)
    ...
    def readParametersFile(self):
        ...
        self.df_bridgit_predictions = self.df_bridgit_predictions[
            self.df_bridgit_predictions['score'] >= self.bridgit_threshold]
    ...
    def getBalancedReactionsDf(self):
        ...
        if self.exclude_unbalanced == 1:
            ...
            df_balance = pd.read_csv(project_config.DATA_DIR / 'reaction_balance.csv')
            ...
            print(len(set(self.df_reactions['rxnUID'].to_list())), " balanced reactions selected out of ",
                  num_reactions_before, "initial reactions")
